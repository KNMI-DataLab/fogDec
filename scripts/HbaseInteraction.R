...
port = 9090  #Default port for thrift service #use 8080 when inside KNMI
...
fileList <- list.files("dataHBASE/twente", full.names = T)

for (file in fileList){
  dataFeatures <- readRDS(file)
  #lastSplit <- length(strsplit(file))
  dataFeatures[, fileName:=sapply(strsplit(filePath, '/'), tail, 1)]
