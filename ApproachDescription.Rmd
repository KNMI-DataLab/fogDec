...
fogTree <- rpart(foggy ~ meanEdge + changePoint + meanBrightness, trainDeBilt , control = rpart.control(cp = 0.019))
```
...
pred <- predict(fogTree, testDeBilt, method="class")
testDeBilt[, pred := pred]
...
fogWindTree <- rpart(foggy ~ meanEdge + changePoint + meanBrightness + windSpeed, trainDeBilt , control = rpart.control(cp = 0.019))
```
...
pred <- predict(fogWindTree, testDeBilt, method="class")
testDeBilt[, pred := pred]
```
...
pred <- predict(mlm, trainDeBilt)
trainDeBilt[, predMOR := pred]
```
