...
```{r, Edges, echo=FALSE, fig.height=4, fig.width=8.4}
...
The tree has to be read as follows for a image with `meanEdge = 0.0039` and 
`changePoint = 320`. From the top node we go to the lower right node,
with number 3, because `meanEdge >= 0.0041` is false. In the next step we go to
the lower left node, with number 6, because `changePoint < 324` is true. 
This node number 6, tells us that there were 25 instances in the training
set and from these 0 were foggy, i.e. `MOR < 250`. Therefore, we get a
prediction of 0\%.
...
in the test set, that are labeled as foggy, i.e. a MOR below 200 meters. The
...
transmission estimate leads to wrong a conclusion. On January 6 it is already
dark (which should have been filtered out prior to the analysis (time zone
issues)). Moreover, the MOR gives values around 750 meters so the roughly 43% 
...
```{r, UncleanPlot, warning=FALSE, echo=FALSE}
...
```{r, plotRegression, echo=FALSE}
ggplot(train, aes(x = log(MOR), y = predMOR)) + geom_point() +
  geom_smooth(method="lm", formula = y ~ splines::bs(x, 3), se=FALSE) +
  #geom_quantile(method="rqss", lambda = 1, quantiles=0.5)
  ylab("prediction")
```
