...
```{r, LookIntoClusters, cache=TRUE, dependson='MultiLinearRegressionTwente'}
...
example_par <- par(mfrow=c(1,2))
img <- lapply(exampleImg, load.image)
plot(img[[1]], main = "Light in the objective")
par(example_par)
...
example_par <- par(mfrow=c(1,2))
img <- lapply(exampleImg, load.image)
plot(img[[1]], main = "Change in scene: light brown pattern in the distance")
plot(img[[2]], main = "Change in scene: full green color in the distance")
par(example_par)
...
exampleImg <- list.files("inst/extdata/cornerCasesTwente/", pattern = "EHTW_20150628")
example_par <- par(mfrow=c(1,3))
img <- lapply(exampleImg, load.image)
lapply(img, plot, main = "Humidity in the objective")
par(example_par)
