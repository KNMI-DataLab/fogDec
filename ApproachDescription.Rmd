...
mostly to already dark hours. After this cleaning, `r train[, .N]` images remain.
...
```{r, PODandFAR, echo=TRUE}

fit <- lm(log(MOR) ~ splines::bs(predMOR, 3), train)
fitted <- predict(fit, train)
train[, fitted := exp(fitted)]

train[MOR < 250 & fitted < 250, .N] / train[MOR < 250, .N]
train[fitted < 250 & MOR > 250, .N] / train[fitted < 250, .N]

train[MOR < 1000 & fitted < 1000, .N] / train[MOR < 1000, .N]
train[fitted < 1000 & MOR > 1000, .N] / train[fitted < 1000, .N]

train[MOR > 3000 & fitted > 3000, .N] / train[MOR > 3000, .N]
train[fitted > 3000 & MOR < 3000, .N] / train[fitted > 3000, .N]

train[MOR > 5000 & fitted > 5000, .N] / train[MOR > 5000, .N]
train[fitted > 5000 & MOR < 5000, .N] / train[fitted > 5000, .N]

```

```{r, plotRegression, echo=FALSE, fig.height=4, fig.width=4}

ggplot(train, aes(x = log(MOR), y = predMOR)) + geom_point() +
  geom_smooth(method="lm", formula = y ~ splines::bs(x, 3), se=FALSE) +
  geom_vline(xintercept = c(log(250), log(1000), log(8000)), lty = 3) + 
  ylab("modelled log(MOR)")
```

```{r, include=FALSE}
# The sequence around June 16 2015 at 13:10 shows the auto adaptation of the
# camera. 
# Similar at September 4 2015 at 11:00. 
# November 22 2015 at least since 14:00 the scenery is different.
```
...
```{r, plotRegressionValidation, echo=FALSE, warning=FALSE, fig.height=4, fig.width=4}
...
fitted <- predict(fit, testClean)
testClean[, fitted := exp(fitted)]

testClean[MOR < 250 & fitted < 250, .N] / testClean[MOR < 250, .N]
testClean[fitted < 250 & MOR > 250, .N] / testClean[fitted < 250, .N]

testClean[MOR < 1000 & fitted < 1000, .N] / testClean[MOR < 1000, .N]
testClean[fitted < 1000 & MOR > 1000, .N] / testClean[fitted < 1000, .N]

testClean[MOR > 3000 & fitted > 3000, .N] / testClean[MOR > 3000, .N]
testClean[fitted > 3000 & MOR < 3000, .N] / testClean[fitted > 3000, .N]

testClean[MOR > 5000 & fitted > 5000, .N] / testClean[MOR > 5000, .N]
testClean[fitted > 5000 & MOR < 5000, .N] / testClean[fitted > 5000, .N]
```
```
