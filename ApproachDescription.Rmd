...
```{r, Edges, cache=FALSE, include=FALSE, fig.height=4, fig.width=8.4}
...
```{r, TransmissionPlotClear, cache = TRUE, echo=FALSE, fig.cap="Transmission in a clear situation", fig.height=3, fig.width=8.4}
im <- subim(load.image(filenames[75]), y > 16)
...
trainDeBilt <- imageSummary[dateTime > sunriseDateTime & dateTime < sunsetDateTime, ]
trainDeBilt[, foggy := MOR < 250]
setkey(trainDeBilt, dateTime)
trainDeBilt <- trainDeBilt[windDeBilt, nomatch = 0]
trainDeBilt <- trainDeBilt[humidityDeBilt, nomatch = 0]
trainDeBilt <- trainDeBilt[, .(dateTime, meanEdge, changePoint, smoothness, meanHue,
                meanSaturation, meanBrightness, MOR, foggy, windSpeed, relHumidity) ]
trainDeBilt <- na.omit(trainDeBilt) #[complete.cases(trainDeBilt)]
...
testDeBilt <- imageSummary[dateTime > sunriseDateTime & dateTime < sunsetDateTime]
testDeBilt[, foggy := MOR < 250]
setkey(testDeBilt, dateTime)
testDeBilt <- testDeBilt[windData, nomatch = 0]
testDeBilt <- testDeBilt[humidityDeBilt, nomatch = 0]
testDeBilt <- na.omit(testDeBilt)
...
```{r, MultiLinearRegressionWind, echo=FALSE}
...
```{r, UncleanPlotWind, warning=FALSE, echo=FALSE, fig.height=4, fig.width=4}
ggplot(trainDeBilt, aes(x = log(MOR), y = predMOR)) + geom_point() +
  geom_vline(xintercept = c(log(250), log(1000), log(3000), log(5000)), lty = 3) + 
  ylab("modelled log(MOR)")
```
...
```{r, LoadTwente}
load("./results/ResultsTwente2015_3hSun.RData")
trainTwente <- imageSummary[dateTime > sunriseDateTime & dateTime < sunsetDateTime, ]
trainTwente[, foggy := MOR < 250]
trainTwente <- na.omit(trainTwente)

load("./results/ResultsTwente2016_3hSun.RData")
testTwente <- imageSummary[dateTime > sunriseDateTime & dateTime < sunsetDateTime, ]
testTwente[, foggy := MOR < 250]
testTwente <- na.omit(testTwente)
...
Based on the training set we obtain the following preliminary decision tree:
```{r, ClassificationTwente, include=FALSE}


fogTree <- rpart(foggy ~ meanEdge + changePoint + meanBrightness, trainTwente , control = rpart.control(cp = 0.019))
```

```{r, ClassificationTreeTwente, echo=FALSE}
fancyRpartPlot(fogTree, sub="")
```

```{r,EvaluatePredictionTwente}
...
```{r, MultiLinearRegressionTwente, echo=FALSE}
...
```{r, UncleanPlotTwente, warning=FALSE, echo=FALSE, fig.height=4, fig.width=4}
