...
#### Outlier detection
```{r Outliers, dependson='dataDeBilt'}
tmp <- dataDeBilt[, changePoint]
multiplier <- qcauchy(0.98) #ceiling(qnorm(0.99999)*10)/10 

mean(tmp) + multiplier * sd(tmp)
mean(tmp) - multiplier * sd(tmp)

dataDeBilt[changePoint > mean(tmp) + multiplier * sd(tmp), .(dateTime, changePoint, MOR)]
dataDeBilt[changePoint < mean(tmp) - multiplier * sd(tmp), .(dateTime, changePoint, MOR)]
...
```{r, cache=TRUE, dependson='trainTestNoRainTwente'}
