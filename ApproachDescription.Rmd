...
```{r, CaretConfusionDeBilt, cache = TRUE, dependson='CaretClassificationDeBilt'}
...
```{r, ImagesExampleTwente, cache=TRUE, echo=FALSE}
...
```{r, PrecipitationDataTwente, cache=TRUE}
...
```{r, trainTestDataTwente, cache=TRUE, dependson=c('TemperatureDataTwente', 'HumidityDataTwente', 'WindDataTwente', 'dataTwente', 'PrecipitationDataTwente')}
...
```{r, ClassificationTwente, include=FALSE, cache=TRUE, dependson='trainTestDataTwente'}
...
```{r, MeteoFeatureAdditionTwente, cache=TRUE, dependson='trainTestDataTwente'}
...
```{r, TestWithWindTw, cache=TRUE, dependson=c('trainTestDataTwente', 'MeteoFeatureAdditionTwente')}
...
```{r, TwentePrecipitationTrainTree, cache=TRUE, echo=FALSE, dependson='trainTestDataTwente'}
...
```{r, ClassificationTreeWithPrecipitationdTw, echo=FALSE, cache=TRUE, dependson='TwentePrecipitationTrainTree'}
fancyRpartPlot(fogTreeRainTwente, sub="")
```


```{r, ResultsPrecipitation, dependson=c('TwentePrecipitationTrainTree', 'TrainTestDataTwente')}
...
```{r, TwenteBinaryPrecipitationTrainTree, cache=TRUE, echo=FALSE, dependson='trainTestDataTwente'}
...
```{r, ClassificationTreeWithBinaryPrecipitationdTw, echo=FALSE, cache=TRUE, dependson='TwenteBinaryPrecipitationTrainTree'}
fancyRpartPlot(fogTreeBinaryRainTwente, sub="")
```


```{r, ResultsBinaryPrecipitation, echo=FALSE, cache=TRUE, dependson=c('TwenteBinaryPrecipitationTrainTree', 'TrainTestDataTwente')}
...
Inspecting the false positives caused by water drops in the  objective  we can recognize that this situation takes place when there is a sustained period of rain (it is reported rainy for a full 10-minute period in a 10 minutes sample) and the wind is higher than 3.5 m/s.
We remove then such conditions from the dataset and we also remove the day when the MOR was not working properly giving visibility below 250m but visual inspection showed perfect conditions.
```{r, cleanRainTwente, echo=FALSE, cache=TRUE, dependson='trainTestDataTwente'}
...
removedFoggyRain <- dim(dataTwente[foggy==TRUE,])[1]-dim(noRainDataTwente[foggy==TRUE,])[1]
```

The dataset is now composed of `r dim(noRainDataTwente)[1]` pictures. With such a filtering procedure also `r removedFoggyRain` foggy pictures have been removed from the dataset.

------HAVE TO CHECK THE PERFOMANCE IN THE NEW SITUATION AFTER REMOVAL OF RAIN-----

We partition again the the dataset and train a decision tree with image and meteorological features.

```{r, trainTestNoRainTwente, echo=FALSE, cache=TRUE, dependson='cleanRainTwente'}
...
```{r, ClassificationTwenteNoRain, include=FALSE, cache=TRUE, dependson='trainTestDataNoRainTwente'}
...
```{r, ClassificationTreeTwenteNoRain, echo=FALSE, cache=TRUE, dependson='ClassificationTwenteNoRain'}
fancyRpartPlot(fogTreeNoRain, sub="")
```

The accurancy of the classification is as follows for the :
```{r, EvaluatePredictionTwenteNoRain, cache=TRUE, dependson=c('ClassificationTwenteNoRain','trainTestNoRainTwente')}
