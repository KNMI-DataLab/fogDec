##Code thanks to the generosity of Hannah Fry
...
#Read in from a file
BNG = csv.reader(open('cameraimages.csv', 'rU'), delimiter = ',')
BNG.next()

#Get the output file ready
outputFile = open('scotlandLatLon.csv', 'wb')
output=csv.writer(outputFile,delimiter=',')
output.writerow(["locationID", "stationID", "locationName", "lon", "lat", "filePattern", "imagePrefix", "imageFormat", "fileLocation"])

#Loop through the data
id=1
for name, prefixCamera, E, N in BNG:
 if "M90" in name:
    lat, lon = OSGB36toWGS84(float(E), float(N))
    locationID = prefixCamera.rsplit(".",1)[0]
    output.writerow(["UK"+str(id), locationID, name,  str(lon), str(lat), "na*me_yyyymmddhhmm.jpg", locationID,"jpg","~/efs/scotland" ])
    id += 1 
#2,290_A_a,Twente,6.89,52.276944,na*me_yyyymmddhhmm.jpg,EHTW,jpg,~/efs/test
