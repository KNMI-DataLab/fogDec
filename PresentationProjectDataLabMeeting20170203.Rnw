...
\title[Fog detection]{Experiences of fog detection using cameras at KNMI}
\subtitle{Working meeting with RWS}
...
% 1.	You will get 10 MINUTES only
% 2.	You can choose to present in English or Dutch (also for slides) 
% 3.	Start with a slide that present: title, purpose and scope, potential (KNMI) benefits, DataLab capability and deliverables of the project (briefly!).
% Note1: specifically mention the potential benefits for KNMI. Besides business benefits, there can be societal, science/knowledge and technology/infrastructuur benefits.
% Note2: indicate at capability whether KNMI DataLab is capable to support the project (idea) with the right skills, data and tooling.     
% 4.	Present a slide with the status and progress of the project  (e.g. list successes and obstacles).
% 5.	Present some 2-3 detail slides(s) about the project (e.g. background, approach, activities etc).
...
\begin{itemize}
\item Purpose: use cameras to identify fog conditions to issue safety hazards
\item Scope: daylight fog identification from static cameras using image analysis
\item Benefits:
\begin{itemize}
\item Society: 
\begin{itemize}
...
Maturing more knowledge and confidence in ML techniques.

\begin{columns}
\column{0.65\textwidth}
\begin{itemize}
\item{Maturing more knowledge and confidence in ML techniques}
\item{Confluence pfoject repository}
...
\begin{frame}{Machine Learning: De Bilt classification tree\footnote{in the following daily conditions only}}
...
\begin{frame}{Fog warning without new infrastructure?}

\begin{textblock*}{5cm}(1cm,2.5cm)
