...
\date{April 9, 2018}
\author[Pagani, Noteboom, Wauben]{G.A. Pagani, J.W. Noteboom and W. Wauben}
...
\begin{frame}{Visibility sensors vs. traffic cameras}
...
\begin{itemize}
\item New more general idea of fog detection than our previous attempt with decision trees and picture features
\item Reason to use: scenery are too different also for the same camera (e.g., zoom)
...
modelDarch<-readRDS("~/development/fogNNmodels/NNmodelTrainedWithStationCouplingEGU.RDS")
...
\item Training set: \Sexpr{length(sum(training$foggy))} +-5600 random fog and \Sexpr{length(sum(training$foggy==FALSE))}+-5600 non-fog pictures from 96 cameras 
\item Test set: \Sexpr{length(sum(testing$foggy))} +-2400 fog and  \Sexpr{length(sum(testing$foggy==FALSE))} non-fog pictures
...
\begin{textblock*}{8cm}(7.5cm,2.2cm)
\begin{center}
\includegraphics[width=2.3cm]{training.png}\\
\includegraphics[width=2.3cm]{test.png}
\end{center}
\end{textblock*}
\end{frame}
...
draw_confusion_matrix_binary(confInSample)
@
\end{itemize}
\end{itemize}
\end{frame}
