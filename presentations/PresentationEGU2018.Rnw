...
\item Training set: +-\Sexpr{round(sum(training$foggy/100))*100} random fog and +-\Sexpr{round(sum(training$foggy==FALSE)/100)*100} non-fog pictures from \Sexpr{length(unique(training$camera_id))} cameras 
\item Test set: +-\Sexpr{round(sum(testing$foggy)/100)*100} fog and  +-\Sexpr{round(sum(testing$foggy==FALSE)/100)*100} non-fog pictures
...
\item Manual labelling of the pictures far from meteo stations (labor intensive, any volunteers??)
