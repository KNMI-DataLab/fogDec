...
\end{frame}
% 
\begin{frame}{How to handle +100k images?}
...
\begin{itemize}
\item +2.8 million images archived
\item Initial investigation in domain adaptation fog detection:
\begin{itemize}
...
\begin{frame}{Neural Network (nutshell)}
...
\item Labels: from MOR sensor, fog considered if visibility $\leq$250m
\begin{itemize}
