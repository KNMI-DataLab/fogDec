...
\begin{frame}{Example Feature: Edge detection}
\vspace*{-5mm}
...
\begin{frame}{Example ML algorithm: Classification tree\footnote{in the following daily conditions only}}
...
\begin{tcolorbox}[width=6cm, left=-0.2cm, right=-0.1cm,enhanced,drop shadow southwest,sharp corners] %{width=5cm, left=7.2cm, top=2cm}
...
\begin{itemize}
\item{Archiving images of 300 cameras around Scotland every 10 minutes}
...
\begin{itemize}
\item{Image analysis code in R/C++}
\item{Heavy use of parallel computation in R}
\item{Parallel cluster on Amazon AWS cloud (20 nodes)}
...
\begin{frame}{Next steps}
 ...
 \begin{block}{\alert{Hopefully} more images to test algorithms}
 \end{block}
 
 \begin{block}{More research into features and different ML algorithms}
 \end{block}
 
 \begin{block}{Development of a spatio-temporal model}
 \end{block}
