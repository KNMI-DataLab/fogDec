...
\begin{frame}{Contingency tables}\centering
 \begin{tabular}{rlrrr}\centering
   & Probability / MOR    & $< 250$m                          &     $\geq 250$m  &  Total\\[2mm]
  2015 & $> 0.5$       & \Sexpr{train[MOR < 250 & pred > 0.5, .N]}  & \Sexpr{train[MOR >= 250 & pred > 0.5, .N]} & \Sexpr{train[pred > 0.5, .N]} \\
  ...
  2016 & $> 0.5$       & \Sexpr{test[MOR < 250 & pred > 0.5, .N]}  & \Sexpr{test[MOR >= 250 & pred > 0.5, .N]} & \Sexpr{test[pred > 0.5, .N]} \\
...
%image is very clear.
 ...
 \begin{itemize}
  ...
  \item still a few outliers can be detected already
 \end{itemize}
\end{frame}
...
\begin{frame}{Outlook}
 ...
 \begin{itemize}
  \item use additional features, e.g. wind or dewpoint temperature
